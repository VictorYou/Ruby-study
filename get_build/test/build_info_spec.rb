...
describe "test BuildInfo class to get build hash" do
  it "should fetch web page from url and save it to a local file" do
    RemoteUrl.expects(:system).with('rm -rf consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047').returns(true)
    RemoteUrl.expects(:system).with('rm -rf consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047').returns(true)
    RemoteUrl.expects(:system).with("wget -q --no-check-certificate 'https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047/timestamps/?time=HH:mm:ss&appendLog' -O consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047").returns(true)
    RemoteUrl.expects(:log).returns(true)
    RemoteUrl.expects(:log).returns(true)
    expect(RemoteUrl.get_url('https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047/timestamps/?time=HH:mm:ss&appendLog', 'consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047') {|f| f}).to eq 'consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047'
  end

  # this saves time to further check labactions that are deleted
  it "should raise exception when cannot get lab url" do
    url = 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20180119_194851_493057&lab=CloneCLAB2774&project=nass'
    ...
    expect {JenkinsBuildInfo.get_labaction_url_html('http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20180119_194851_493057&lab=CloneCLAB2774&project=nass') {|f| f}}.to raise_error(Exception, "error accessing lab action page")
  end

  it "should form correct file name from url for sonar bulds" do
    @tester = SonarBuildInfo.new({})
    expect(@tester.file_from_url('https://eseecisav71.emea.ms-net.net:9000/api/measures/component_tree?asc=false&ps=100&metricSortFilter=withMeasuresOnly&p=1&s=metric%2Cname&metricSort=test_execution_time&baseComponentKey=com.ko.oss.isdk.mediation.ftppm%3Aisdk-ftp-pm&metricKeys=test_execution_time&strategy=leaves')).to eq 'sonar.com.ko.oss.isdk.mediation.ftppm.isdk-ftp-pm'
  end

  it "should form correct file name from url and call get_url" do
    @tester = JenkinsBuildInfo.new({})
    @tester.expects(:log).returns true
    expect(@tester.file_from_url('https://eslinb87.emea.ms-net.net:8080/view/Gate/job/GATE-adap_core1-component-build-deploy-test/93//timestamps/?time=HH:mm:ss&appendLog')).to eq 'consoleText.eslinb87.emea.ms-net.net.GATE-adap_core1-component-build-deploy-test.93'
  end

  it "should return false when invalid url is given" do
    @tester = JenkinsBuildInfo.new({})
    @tester.expects(:log).returns true
    expect(@tester.file_from_url('https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/')).to eq ""
  end
  ...
  it "should read content file and return info" do
    file = 'consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047'
    @tester = JenkinsBuildInfo.new({})
    content = IO.read(file)
    content.gsub!(/\n/, '')
    upgrade_url = 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_083843_180224&lab=CloneCLAB2890&project=nass'
    test_url = 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_100718_206973&lab=CloneCLAB2890&project=nass'
  ...
  it "parse res_id giving content" do
    ...
    content = IO.read('consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047')
    content.gsub!(/\n/, '')
    expect(@tester.parse_res_id(content)).to eq 'nass/smu_platform1_git/18.0.0.96'

    content = IO.read('consoleText.eslinb87.emea.ms-net.net.GATE-radio3lte_static-component-build-deploy-test.17')
    content.gsub!(/\n/, '')
    expect(@tester.parse_res_id(content)).to eq '=res_id==radio3_lte_com.ms.netact.man-8_17.8_17'
  end

  it "should read url and return test duration" do
    @tester = JenkinsBuildInfo.new({})
    url = 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_100718_206973&lab=CloneCLAB2890&project=nass'
  ...
  it "should read content file and return upgrade url" do
    ...
    content = IO.read('consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047')
    content.gsub!(/\n/, '')
    expect(@tester.get_upgrade_url(content)).to eq 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_083843_180224&lab=CloneCLAB2890&project=nass'

    # test for exp pipeline
    content = IO.read('consoleText.eslinb87.emea.ms-net.net.EXP-component-experimental-test.1492')
    content.gsub!(/\n/, '')
    expect(@tester.get_upgrade_url(content)).to eq 'http://brigida.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20180112_042240_311621&lab=CloneCLAB2275&project=N8_labs'
  end
  ...
  it "should read content file and return test url" do
    ...
    content = IO.read('consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047')
    content.gsub!(/\n/, '')
    expect(@tester.get_test_url(content)).to eq 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_100718_206973&lab=CloneCLAB2890&project=nass'

    # test for exp pipeline
    content = IO.read('consoleText.eslinb87.emea.ms-net.net.EXP-component-experimental-test.1492')
    content.gsub!(/\n/, '')
    expect(@tester.get_test_url(content)).to eq 'http://brigida.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20180112_050729_547028&lab=CloneCLAB2275&project=N8_labs'
  ...
  it "should read upgrade url and return upgrade duration for gate pipeline" do
    @tester = JenkinsBuildInfo.new({})
    @tester.expects(:log).returns(true)
    url = 'http://dome.netact.ms-rdnet.net/mpp/dynamic/logs/lab_action?action_dir=20170720_083843_180224&lab=CloneCLAB2890&project=nass'
  ...
  it "should read correct end time when it is in a different line than result" do
    file = 'consoletext.eslinb86.emea.ms-net.net.GATE-NBI_INVENTORY-build-deploy-test.6'
    @tester = JenkinsBuildInfo.new({})
    content = IO.read(file)
    content.gsub!(/\n/, '')
    result = $1 if /Finished: (FAILURE|ABORTED|SUCCESS)/ =~ content
  ...
  it "should read correct end time when it is in the same line as result" do
    file = 'consoleText.eslinb87.emea.ms-net.net.EXP-component-experimental-test.1492'
    @tester = JenkinsBuildInfo.new({})
    content = IO.read(file)
    content.gsub!(/\n/, '')
    result = $1 if /Finished: (FAILURE|ABORTED|SUCCESS)/ =~ content
  ...
  it "should return build info by giving jenkins and gate" do
    ...
    @tester.expects(:build_info_url).returns 'https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047'
  ...
  it "should return build url by giving server, job and build number" do
    @tester = JenkinsBuildInfo.new(:type=>'GATE', :js=>'eslinb86.emea.ms-net.net', :jb=>'GATE-nass-component-build-deploy-test', :bn=>1047)
    expect(@tester.build_info_url).to eq 'https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047/timestamps/?time=HH:mm:ss&appendLog'
  end

  it "should merge type, build url etc for jenkins build" do
    @tester = JenkinsBuildInfo.new(:type =>'GATE')
    build_time_url = 'https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047'
  ...
  it "should initialize build hash list correctly" do
    @tester = AllBuildInfo.new(['eslinb86.emea.ms-net.net'], ['GATE-med_core3-component-build-deploy-test', 'EXP-component-experimental-test', 'CHECK-component-build-unit-test'])
  ...
  it "should initialize build hash list correctly" do
    @tester = JenkinsBuildInfoList.new(:jenkins_list=>['eslinb86.emea.ms-net.net'], :job_list=>['GATE-med_core3-component-build-deploy-test', 'EXP-component-experimental-test', 'CHECK-component-build-unit-test'], :type=>'GATE')
  ...
  it "should return last build number by giving jenkins and gate" do
    ...
    expect(JenkinsBuildInfoList.last_build_number('eslinb86.emea.ms-net.net', 'GATE-release-upgrade-NA17-2-verification')).to eq 1047
  end

  it "should save 1 build info" do
    @tester = JenkinsBuildInfoList.new(:jenkins_list=>['eslinb86.emea.ms-net.net'], :job_list=>['GATE-nass-component-build-deploy-test'], :type=>2)
    build_info = {:result => 'SUCCESS'}
    
    @tester.save_1_build_info({:type=>'GATE', :js=>'eslinb86.emea.ms-net.net', :jb=>'GATE-nass-component-build-deploy-test', :bn=>1047}, build_info)
  ...
  it "should set common variable to be false if there is exception when saving 1 build info" do
    ...
    @tester.expects(:create_thread_key).returns('eslinb86.emea.ms-net.netGATE-nass-component-build-deploy-test')
    @tester.save_build_info_1_build(:type=>'GATE', :js=>'eslinb86.emea.ms-net.net', :jb=>'GATE-nass-component-build-deploy-test', :bn=>1047){|options| raise "an exception processing build info"}
    expect(@tester.instance_eval{@create_thread['eslinb86.emea.ms-net.netGATE-nass-component-build-deploy-test']}).to eq false
  end

  it "should get build duration giving url" do
    @tester = JenkinsBuildInfo.new({})
    RemoteUrl.expects(:get_url).with('https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047/', 'consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047.time').returns ({:duration=>115, :date=>Time.new(2017,'Jul',20,05,34,26)})
    expect(@tester.get_build_time('https://eslinb86.emea.ms-net.net:8080/job/GATE-nass-component-build-deploy-test/1047/')).to eq ({:duration=>115, :date=>Time.new(2017,'Jul',20,05,34,26)})
  end

  it "should parse build duration giving file" do
    @tester = JenkinsBuildInfo.new({})
    DurationDecoder.expects(:time_in_min).with('1 hr 55 min').returns(115)
    expect(@tester.parse_build_time('consoleText.eslinb86.emea.ms-net.net.GATE-nass-component-build-deploy-test.1047.time')).to eq ({:duration=>115, :date=>Time.new(2017,'Jul',20,05,34,26)})
  end
  ...
  it "should return create thread key as jenksins server + jenkins job" do
    @tester = JenkinsBuildInfoList.new(:jenkins_list=>['eslinb86.emea.ms-net.net'], :job_list=>['GATE-med_core3-component-build-deploy-test', 'EXP-component-experimental-test', 'CHECK-component-build-unit-test'], :type=>'GATE')
    expect(@tester.create_thread_key({:js=>'eslinb86.emea.ms-net.net', :jb=>'GATE-med_core3-component-build-deploy-test'})).to eq 'eslinb86.emea.ms-net.netGATE-med_core3-component-build-deploy-test'
  end
  ...
  it "should clone repo if it does not exist locally" do
    ...
    expect(get_repo_cmd('isdk_ftp_pm')).to eq 'git clone ssh://viyou@gerrite1.ext.net.ko.com:8282/netact/isdk_ftp_pm /home/viyou/repo/isdk_ftp_pm'
  end
  ...
  it "should get group id and artificat id from a pom file" do
    ...
    expect(@tester.get_id_from_pom('pom.xml')).to eq ({:gId=>'com.ko.oss.isdk.mediation.ftppm', :aId=>'isdk-ftp-pm'})
  end

  it "should get group id from parent if it does not have" do
    @tester = SonarBuildInfoList.new({:project_list=>[]})
    expect(@tester.get_id_from_pom('pom_gId_parent.xml')).to eq ({:gId=>'com.ms.oss.nbi3gcpm.ftirp', :aId=>'pm-report-handler-su'})
  end

  it "should get correct build url for sonar build" do
    @tester = SonarBuildInfo.new({:gId=>'com.ko.oss.isdk.mediation.ftppm', :aId=>'isdk-ftp-pm'})
    expect(@tester.build_url).to eq 'https://eseecisav71.emea.ms-net.net:9000/component_measures/metric/test_execution_time/list?id=com.ko.oss.isdk.mediation.ftppm%3Aisdk-ftp-pm'
  end

  it "should get correct build info url for sonar build" do
    @tester = SonarBuildInfo.new({:gId=>'com.ko.oss.isdk.mediation.ftppm', :aId=>'isdk-ftp-pm'})
    expect(@tester.build_info_url).to eq 'https://eseecisav71.emea.ms-net.net:9000/api/measures/component_tree?asc=false&ps=100&metricSortFilter=withMeasuresOnly&p=1&s=metric%2Cname&metricSort=test_execution_time&baseComponentKey=com.ko.oss.isdk.mediation.ftppm%3Aisdk-ftp-pm&metricKeys=test_execution_time&strategy=leaves'
  end
  ...
  it "should parse ut file and time from file" do
    ...
    expect(@tester.parse_build_info('sonar.com.ko.oss.isdk.mediation.ftppm.isdk-ftp-pm')).to eq [{:file=>'src/test/java/com/ms/oss/mdk/collectors/ftp/FTPHelperImplTest.java', :ut_duration=>156, :url=>'https://eseecisav71.emea.ms-net.net:9000/component_measures/metric/test_execution_time/list?id=com.ko.oss.isdk.mediation.ftppm%3Aisdk-ftp-pm'}, {:file=>'src/test/java/com/ms/oss/mdk/pm/converter/OMeSConverter3GPPV6_2_DTDTest.java', :ut_duration=>103, :url=>'https://eseecisav71.emea.ms-net.net:9000/component_measures/metric/test_execution_time/list?id=com.ko.oss.isdk.mediation.ftppm%3Aisdk-ftp-pm'}]
  end
